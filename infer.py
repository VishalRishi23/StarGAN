...
class Model:
    def __init__(self, save_path):
        ...
        # load weights
        self.generator.load_state_dict(torch.load(os.path.join(save_path, "generator.pth"),map_location=torch.device('cpu')))
        self.mapping_network.load_state_dict(torch.load(os.path.join(save_path, "mapping_network.pth"),map_location=torch.device('cpu') ))
        self.style_encoder.load_state_dict(torch.load(os.path.join(save_path, "style_encoder.pth"),map_location=torch.device('cpu')))
        self.face_mask.load_state_dict(torch.load(os.path.join(save_path, "facemask.pth"),map_location=torch.device('cpu')))
...
if __name__ == "__main__":
    ...
    src = load_img(os.path.join('sample dataset','female',"000168.jpg"))
    ref = load_img(os.path.join('sample dataset','female',"008210.jpg"))
    style = model.get_style(ref, np.array([0]))
